...
import 'package:facial_attendance/utility/size_config.dart';
...
class RecogniseFirst extends StatefulWidget {
  @override
  _RecogniseFirstState createState() => _RecogniseFirstState();
}

class _RecogniseFirstState extends State<RecogniseFirst> {
  ...
  Future<void> _captureImage(context) async {
    ...
    _uploadMedia(context, File(image.path), "image");
  }

  Future<void> _recordVideo(context) async {
    if (_controller == null || !_controller!.value.isInitialized) return;
    ...
    if (response.statusCode == 200) {
      ...
      _showAlertDialog(
          ...
          jsonResponse['status'] == 'success'
              ? "Face Mathced as ${jsonResponse['user_name']}"
  ...
  void _showAlertDialog(BuildContext dialogContext, String title,
        ...
        return AlertDialog(
          title: Text(title,
              style: TextStyle(
                  ...
                  color:
                      title == "Checked out" ? appThemeColor : appGreenColor)),
          content: Text(
            message,
            style:
                TextStyle(fontSize: getScreenWidth(18), fontFamily: ffGMedium),
          ),
          // backgroundColor: isSuccess ? Colors.green[200] : Colors.red[200],
